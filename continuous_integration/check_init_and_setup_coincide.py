...
def main() -> int:
    ...
    if setup_py_map["version"] != elvolantevirtual.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in oneleggamepad/__init__.py is: "
            f"{elvolantevirtual.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != elvolantevirtual.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in oneleggamepad/__init__.py is: "
            f"{elvolantevirtual.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != elvolantevirtual.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in oneleggamepad/__init__.py is: "
            f"{elvolantevirtual.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != elvolantevirtual.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in oneleggamepad/__init__.py is: "
            f"{elvolantevirtual.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != elvolantevirtual.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in oneleggamepad/__init__.py, "
                f"but found: {elvolantevirtual.__status__}"
            )
            success = False
